...
export class Avatar implements OnInit, OnChanges, AfterViewInit {
  ...
  private _setAutomaticColor(color: any): void {
    ...
    element.style.setProperty('--mf-avatar-bg', color);
    element.style.setProperty('--mf-avatar-border-color', this._newShade(color, -25));
    element.style.setProperty('--mf-avatar-color', this._newShade(color, -150));
  }
