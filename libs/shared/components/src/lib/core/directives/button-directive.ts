...
@Directive({
  selector: '[mfButtonLoading]',
  exportAs: 'mfButtonLoading'
})
export class ButtonDirective implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['loading'].currentValue) {
      this._matButton._elementRef.nativeElement.classList.add('mf-button-loading');
      this._matButton.disabled = true;
      this.createSpinner();
    } else if (!changes['loading'].firstChange) {
      this._matButton._elementRef.nativeElement.classList.remove('mf-button-loading');
