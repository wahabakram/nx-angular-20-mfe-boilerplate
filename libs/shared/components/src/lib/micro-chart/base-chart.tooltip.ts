...
import { OverlayPosition, OverlayPositionManager } from '../overlay';
...
export abstract class BaseChartTooltip {
  ...
  protected _createTooltipOrigin(): void {
    let tooltipOrigin: HTMLElement | null = this._document.querySelector('.mf-mchart-tooltip-origin');

    if (!tooltipOrigin) {
      tooltipOrigin = this._renderer.createElement('div') as HTMLElement;
      tooltipOrigin.classList.add('mf-mchart-tooltip-origin');
  ...
  protected _getOverlayConfig() {
    return new OverlayConfig({
      panelClass: 'mf-mchart-tooltip-overlay',
