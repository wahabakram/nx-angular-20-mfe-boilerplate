...
import { OverlayPosition, OverlayPositionManager } from '../../design/overlay';
...
export abstract class BaseChartTooltip {
  ...
  protected _createTooltipOrigin(): void {
    let tooltipOrigin: HTMLElement | null = this._document.querySelector('.mfc-mchart-tooltip-origin');

    if (!tooltipOrigin) {
      tooltipOrigin = this._renderer.createElement('div') as HTMLElement;
      tooltipOrigin.classList.add('mfc-mchart-tooltip-origin');
  ...
  protected _getOverlayConfig() {
    return new OverlayConfig({
      panelClass: 'mfc-mchart-tooltip-overlay',
