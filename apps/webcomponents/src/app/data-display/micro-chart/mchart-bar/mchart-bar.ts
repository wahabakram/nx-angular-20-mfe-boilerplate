...
import { OverlayPosition } from '../../../design/overlay';

let nextId = 0;

@Component({
  selector: 'mfc-mchart-bar',
  exportAs: 'mfcMchartBar',
  templateUrl: './mchart-bar.html',
  styleUrl: './mchart-bar.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: {
    class: 'mfc-mchart-bar',
    '[class.fill-gradient]': 'fillGradient()',
    '[class.with-tooltip]': '!!tooltip()',
  },
})
export class MchartBar
  extends BaseChartTooltip
  ...
  private _setupData(): void {
    ...
    if (!this._dataContainer) {
      this._dataContainer = this._svg
        ...
        .attr('transform', `translate(0,0)`);
      const allBars = this._dataContainer
        .selectAll('rect')
        .data(this.data())
        .enter()
        .append('rect')
    ...
    fromEvent<MouseEvent>(this._document, 'mousemove')
      ...
      .subscribe((e: MouseEvent) => {
        ...
        if (
          posX < 0 ||
          posY < 0 ||
          posX > this._hostWidth ||
          posY > this._hostHeight
        ) {
          visible = false;
          oldX = null;
          ...
          if (
            !target.closest('.mfc-mchart-line') ||
            !target.classList.contains('mfc-mchart-tooltip-overlay') ||
            !target.closest('.mfc-mchart-tooltip-overlay')
          ) {
            oldX = null;
            this._overlayRef?.detach();
          }
