...
@Component({
  selector: 'mfc-avatar,[mfc-avatar]',
  exportAs: 'mfcAvatar',
...
export class Avatar implements OnInit, OnChanges, AfterViewInit {
  ...
  private _setAutomaticColor(color: any): void {
    ...
    element.style.setProperty('--mfc-avatar-bg', color);
    element.style.setProperty('--mfc-avatar-border-color', this._newShade(color, -25));
    element.style.setProperty('--mfc-avatar-color', this._newShade(color, -150));
  }
