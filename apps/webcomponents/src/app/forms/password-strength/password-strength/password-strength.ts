...
@Component({
  selector: 'mfc-password-strength',
  exportAs: 'mfcPasswordStrength',
  imports: [
    MatProgressBar
  ],
...
export class PasswordStrength implements OnInit, OnChanges, AfterContentChecked, ControlValueAccessor {
  ...
  setRulesAndValidators(): void {
    ...
    this.passwordConfirmationFormControl
      // @ts-expect-error ignore Validators.compose type check
    ...
    this.criteriaMap.forEach((value: any, key: string) => {
      // @ts-expect-error ignore this.validatorsArray.push() type check
  ...
  private _containAtLeastOneLowerCaseLetter(): boolean {
    // @ts-expect-error ignore map get type check
