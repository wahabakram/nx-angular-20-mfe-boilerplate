...
@Component({
  selector: 'mfc-scroll-spy-nav,[mfc-scroll-spy-nav]',
  exportAs: 'mfcScrollSpyNav',
...
export class ScrollSpyNav implements AfterContentInit {
  ...
  ngAfterContentInit() {
    ...
    this._elementToScroll = this._document.querySelector(
      '.mfc-layout-body:has(:not(.mfc-layout-body))'
    ) as HTMLElement;

    if (this._elementToScroll) {
      this._zone.runOutsideAngular(() => {
        fromEvent(this._elementToScroll, 'scroll')
          .pipe(debounceTime(35), takeUntilDestroyed(this._destroyRef))
          .subscribe(() => {
  ...
  scrollTo(targetId: string) {
    ...
    const targetElement = this._document.querySelector(
      '#' + targetId
    ) as HTMLElement;
    const offsetTopFix =
      parseInt(getComputedStyle(targetElement).marginTop) +
      parseInt(getComputedStyle(targetElement).height) +
      this._threshold;
  ...
  private _findActiveItem() {
    for (const item of this._items()) {
      ...
      if (targetElement) {
        if (
          this._elementIsVisibleInViewport(this._elementToScroll, targetElement)
        ) {
