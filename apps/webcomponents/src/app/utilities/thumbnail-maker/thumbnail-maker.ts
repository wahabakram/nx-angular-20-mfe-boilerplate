...
@Component({
  selector: 'mfc-thumbnail-maker',
  exportAs: 'mfcThumbnailMaker',
  imports: [
    MatSlider,
    MatSliderThumb,
    FormsModule,
...
export class ThumbnailMaker {
  ...
  private _dragImage =
    viewChild.required<DragImageDirective>(DragImageDirective);
  ...
  protected increase(): void {
    if ((this.scale + 0.1) * 100 <= this.max) {
      this.scale += 0.1;
    } else {
  ...
  protected decrease(): void {
    if ((this.scale - 0.1) * 100 >= this.min) {
      this.scale -= 0.1;
    } else {
