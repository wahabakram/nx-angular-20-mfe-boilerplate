...
export class Marquee implements AfterContentInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['reverse']) {
      this.nativeElement.style.setProperty('--mfc-marquee-reverse', changes['reverse'].currentValue ? 'reverse' : '');
    }

    if (changes['pauseOnHover']) {
      this.nativeElement.style.setProperty('--mfc-marquee-pause', changes['pauseOnHover'].currentValue ? 'paused' : 'running');
    }
  }
