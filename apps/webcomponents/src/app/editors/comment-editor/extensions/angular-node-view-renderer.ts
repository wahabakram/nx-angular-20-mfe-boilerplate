...
class AngularNodeViewClass extends NodeView<
  Type<AngularNodeView>,
  Editor,
  AngularNodeViewRendererOptions
> {
  renderer!: AngularRenderer<AngularNodeView, NodeViewProps>;
  ...
  override mount() {
    ...
    const props: NodeViewProps = {
      editor: this.editor,
      node: this.node,
      // @ts-expect-error ts-migrate(2322) FIXME: Type 'DecorationWithType[]' is not assignable to type 'D... Remove this comment to see the full error message
...
export const AngularNodeViewRenderer = (
  ViewComponent: Type<AngularNodeView>,
  options: Partial<AngularNodeViewRendererOptions>
): NodeViewRenderer => {
  // @ts-expect-error ts-migrate(2322) FIXME: Type 'DecorationWithType[]' is not assignable to type 'Decoration[]'.
  return (props: NodeViewRendererProps) => {
    return new AngularNodeViewClass(ViewComponent, props, options);
  };
};
