...
@Directive({
  selector: '[mfcButtonLoading]',
  exportAs: 'mfcButtonLoading'
})
export class ButtonDirective implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['loading'].currentValue) {
      this._matButton._elementRef.nativeElement.classList.add('mfc-button-loading');
      this._matButton.disabled = true;
      this.createSpinner();
    } else if (!changes['loading'].firstChange) {
      this._matButton._elementRef.nativeElement.classList.remove('mfc-button-loading');
